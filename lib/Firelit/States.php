...
class States {
	...
	*/
	public static function display($country, $callback, $subset = false) {
		...
		if (!is_callable($callback)) throw new Exception('Callback function is not callable.');
		if ($subset && !is_array($subset)) throw new Exception('Subset must be false or an array of acceptable country abbreviations.');
		
		if (class_exists('Countries') && !Countries::check($country)) return false;
		if (!isset(self::$list[$country])) return false;
		
		foreach (self::$list[$country] as $abbrv => $name) {
			
			if ($subset && !in_array($abbrv, $subset)) continue;
	...
	public static function check($country, $stateAbbrv) {
		...
		if (class_exists('Countries') && !Countries::check($country)) return false;
		if (!isset(self::$list[$country])) return null;
		if (!isset(self::$list[$country][$stateAbbrv])) return false;
	...
	public static function getName($country, $stateAbbrv, $html = true) {
		...
		if (self::check($country, $stateAbbrv)) return false;
