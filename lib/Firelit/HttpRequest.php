...
class HttpRequest {
	...
	private function execute($url) {
		...
		if (self::$config['caInfo']) curl_setopt($this->handle, CURLOPT_SSL_VERIFYPEER, 1); // Verify the SSL certificate
		if (self::$config['followRedirect']) curl_setopt($this->handle, CURLOPT_FOLLOWLOCATION, 1); // Follow redirects
