...
class HttpRequest
{
    ...
    public function __construct()
    {
        ...
        if (!$this->handle) {
            throw new \Exception('Could not initiate cURL.');
        }
    }
    ...
    public function put($url, $putData)
    {
        ...
        if (is_array($putData)) {
            $putData = http_build_query($putData);
        }
    ...
    private function execute($url)
    {
        ...
        } elseif (self::$config['userAgent']) {
            curl_setopt($this->handle, CURLOPT_USERAGENT, self::$config['userAgent']);
        }

        if (self::$config['caInfo']) {
            curl_setopt($this->handle, CURLOPT_CAINFO, self::$config['caInfo']); // Name of the file to verify the server's cert against
        }
        if (self::$config['caInfo']) {
            curl_setopt($this->handle, CURLOPT_SSL_VERIFYPEER, 1); // Verify the SSL certificate
        }
        if (self::$config['followRedirect']) {
            curl_setopt($this->handle, CURLOPT_FOLLOWLOCATION, 1); // Follow redirects
        }
        ...
        $this->respInfo = curl_getinfo($this->handle);
